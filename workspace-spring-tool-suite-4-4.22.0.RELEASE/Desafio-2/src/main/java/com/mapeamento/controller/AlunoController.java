...
public class AlunoController {
    ...
    @ManagedOperation(description = "Localiza o aluno por Cidade")
    public ResponseEntity<List<Aluno>> findAlunosByCidade(@PathVariable String cidade) {
      List<Aluno> alunos = alunoService.findAlunoByCidade(cidade);
      return ResponseEntity.ok(alunos);
    }
    //Query Method
    @GetMapping("/nome/{nome}")
    public ResponseEntity<List<Aluno>> buscarAlunosPorNome(@PathVariable String nome) {
      List<Aluno> alunos = alunoService.findAlunoByNome(nome);
      return ResponseEntity.ok(alunos);
    }
    ...
    public List<Aluno> findAlunosByTurma(@PathVariable Long Id) {
        return alunoService.findByAlunoId(Id);
    }
      
    @GetMapping("/nome/{nome}/renda/{renda}")
    public ResponseEntity<List<Aluno>> findAlunoByNomeAndRenda(@PathVariable String nome,@PathVariable Double renda) {
      List<Aluno> alunos = alunoService.findByNomeAndRenda(nome,renda);
      return ResponseEntity.ok(alunos);
    }
    @GetMapping("/cidade/{cidade}/renda/{renda}")
    public ResponseEntity<List<Aluno>> findAlunosByCidadeAndRenda(@PathVariable String cidade,@PathVariable Double renda) {
      List<Aluno> alunos = alunoService.findByCidadeAndRenda(cidade,renda);
      return ResponseEntity.ok(alunos);
    }
